...
import { probeTarget, sendAttack } from './openclaw'
import { generateAttackVariants } from './nebius'
import { runRecon } from './tavily'
import { getTemplatesForCategory, getTemplatePromptsForCategory, getAllCategories } from './attack-library'
...
export async function runScan(
  ...
  log(emit, 'Planning attack vectors across 4 categories...')

  for (const catResult of categories) {
    const templates = getTemplatesForCategory(catResult.category)
    const templatePrompts = getTemplatePromptsForCategory(catResult.category)
    ...
    try {
      const variants = await generateAttackVariants(
        catResult.category,
        templatePrompts,
        probe.response,
        reconSummary
      ...
      log(emit, `Failed to generate variants for ${catResult.category}: ${msg}`, 'warn')
    }
  ...
  log(emit, `Total: ${totalAttacks} attacks across 4 categories`)
  ...
  for (const catResult of categories) {
    ...
    for (const attack of catResult.attacks) {
      ...
      const response = await sendAttack(config.targetUrl, attack.prompt, config.authToken)
